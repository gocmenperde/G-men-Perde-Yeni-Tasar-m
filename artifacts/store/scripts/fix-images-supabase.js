...
function nameMatchesResult(productName, resultNames) {
  ...
  const keywords = productName.toLowerCase().split(/\s+/).filter(w => w.length > 3);
...
async function findImages(name, barcode, brand) {
  ...
  // 1. Barcode → Trendyol, validate name match
  if (barcode && /^\d{8,14}$/.test(barcode)) {
    const { imgs: bImgs, names: bNames } = await searchTrendyol(barcode);
    if (bImgs.length > 0 && nameMatchesResult(name, bNames)) {
      add(bImgs);
    } else if (bImgs.length === 0) {
      add(await searchN11(barcode));
  ...
  // 2. Name → Trendyol
  if (imgs.length < 1) {
    const { imgs: nImgs } = await searchTrendyol(fullQ);
    add(nImgs);
    await sleep(DELAY_MS);
  }

  // 3. Short name → Trendyol
  if (imgs.length < 1 && fullQ !== name.slice(0, 100)) {
    const { imgs: nImgs2 } = await searchTrendyol(name.slice(0, 80));
    add(nImgs2);
    await sleep(DELAY_MS);
  }
...
async function processRow(row) {
  ...
  if (!isPlaceholderOrBad(images)) return { status: 'skipped' };
...
async function main() {
  log('=== fix-images başladı ===');

  // Get all IDs of products with no images upfront — stable list, no offset drift
  const idRows = psql(`
    SELECT p.id, p.name, p.barcode, p.sku, COALESCE(b.name,''), p.images::text
    FROM public."Product" p
    LEFT JOIN public."Brand" b ON b.id = p."brandId"
    WHERE p."isActive" = true AND (p.images = '{}' OR p.images IS NULL)
    ORDER BY p."createdAt" ASC
  `).split('\n').filter(r => r.trim());

  const total = idRows.length;
  log(`İşlenecek ürün: ${total}`);

  for (let i = 0; i < idRows.length; i += CONCURRENCY) {
    const chunk = idRows.slice(i, i + CONCURRENCY);
    const results = await Promise.all(chunk.map(processRow));
    for (const r of results) {
      if (r.status === 'fixed')      { totalFixed++;    log(`[✓] ${r.name} — ${r.count} resim`); }
      else if (r.status === 'not_found') { totalNotFound++; }
      else if (r.status === 'error') { totalErrors++; log(`[!] ${r.name}: ${r.error}`); }
    }

    if ((i / CONCURRENCY) % 10 === 9) {
      log(`>>> ${Math.min(i + CONCURRENCY, total)}/${total} — Düzeltilen: ${totalFixed} | Bulunamayan: ${totalNotFound}`);
    }
    await sleep(300);
  }
